...
describe("bridge cost telemetry ingestion", () => {
  ...
  it("records nested usage telemetry payload from websocket envelope", async () => {
    const projectId = `proj_bridge_nested_${Date.now()}_${Math.floor(Math.random() * 100000)}`;
    const workingDir = mkdtempSync(join(tmpdir(), "clawos-bridge-cost-"));
    const costsPath = join(workingDir, "cost-store.json");
    const wss = new WebSocketServer({ port: 0 });
    const address = wss.address();
    ...
    try {
      ...
      for (let attempt = 0; attempt < 10; attempt += 1) {
        ...
        expect(summary.status).toBe(200);
        summarySpent = Number(summary.body.spent_usd ?? 0);
        if (summarySpent === 0.31) {
          const agent = summary.body.agents.find((row: { agent_id: string }) => row.agent_id === "sastre");
          expect(agent).toBeDefined();
          expect(agent.tokens_in).toBe(900);
          expect(agent.tokens_out).toBe(450);
          expect(agent.estimated_usd).toBe(0.31);
          break;
        }
